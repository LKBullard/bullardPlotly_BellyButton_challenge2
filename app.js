// Create a horizontal bar chart with a dropdown menu to display the top 10 OTUs found in that individual 
   ...
   function buildCharts(sampleid) {
    ...
    d3.json("samples.json").then((incomingData) => {
        ...
        //Plot chart to a div tag using id "bar"
        ...
        //BONUS: Gauge Chart
        //Had help with this-- didn't test doing it alone //
        var data = [
            {
                domain: { x: [0, 1], y: [0, 1] },
                value: 270,
...
// get demographic info for  id & append
function buildMetadata(sampleid) {
    d3.json("samples.json").then((incomingData) => {
        ...
        //filter metadata info by sampleid
        var metaInfo =metaData.filter(metaData => metaData.id==sampleid)[0]; 
        console.log(metaInfo);

        //Put demographic data here: <div id = "sample-metadata"....</div>
        var demoInfo =d3.select(".panel-body")
        
        //clear input value prior to new user input -- 
        demoInfo.html("");
